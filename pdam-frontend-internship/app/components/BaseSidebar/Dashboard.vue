    ...
    <div class="h-[calc(100vh-20vh)]">
        ...
        <ul :class="{ 'opacity-0 pointer-events-none': !isSidebarLeftOpen && isMobile }">
            <template v-if="permissions.length" v-for="(section, index) in filteredMenuItems" :key="index">
                <li v-if="section && 'label' in section" class="group relative py-2 px-4"
                ...
                >
                    <NuxtLink :to="section.path" class="flex items-center">
                        <Icon :name="section.icon" class="h-5 w-5 min-h-5 min-w-5" :class="{ 'mr-2': isSidebarLeftOpen }" />
                        <span v-if="isSidebarLeftOpen" class="text-[0.7rem] whitespace-nowrap truncate">{{ section.label }}</span>
                    </NuxtLink>
                ...
                <template v-else>
                    ...
                    >
                        <div class="flex items-center justify-between cursor-pointer"
                            @click="item?.children ? toggleDropdown(item.label) : null"
                        >
                        <NuxtLink :to="item?.path"  class="flex items-center">
                            ...
                            <Icon v-if="item?.children && isSidebarLeftOpen"
                                :name="openDropdowns[item.label] ? 'mdi:chevron-up' : 'mdi:chevron-down'"
                                class="w-4 h-4" />
                        </div>
